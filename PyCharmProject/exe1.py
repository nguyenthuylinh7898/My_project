...
read_dataset_dir('C:\\Users\\Thuy Linh\\PycharmProjects\\untitled\\training\\red', 0, red_samples, red_labels)


green_samples, green_labels = [], []
read_dataset_dir('C:\\Users\\Thuy Linh\\PycharmProjects\\untitled\\training\\green', 2, green_samples, green_labels)
...
X_train = np.concatenate([X_red_train, X_green_train], axis=0)
y_train = np.concatenate([y_red_train, y_green_train], axis=0)

X_test = np.concatenate([X_red_test, X_green_test], axis=0)
y_test = np.concatenate([y_red_test, y_green_test], axis=0)
...
def augment_dataset():
    ...
    for augmenting_index in range(n_classes):
        ...
        y_train_augmented = np.concatenate((y_train_augmented, [augmenting_index for i in range(augment_count)]),
                                           axis=0)
...
def batches(batch_size, features, labels):
    ...
    while (sample_index < num_full_batches_samples):
        batches_arr.append(
            [features[sample_index:sample_index + batch_size], labels[sample_index:sample_index + batch_size]])
...
def gen_model(cnn_input, cnn_weights, cnn_strides, cnn_nonlinearities, cnn_maxpooling, mlp_layers, mlp_nonlinearities):
    ...
    for W, s, n, p in zip(cnn_weights, cnn_strides, cnn_nonlinearities, cnn_maxpooling):
        ...
        cnn_layer_input = tf.nn.max_pool(cnn_layer_input, ksize=[1, p[0], p[1], 1], strides=[1, p[0], p[1], 1],
                                         padding='SAME')
...
cnn_layers = [[5, 5, 3, 16], [5, 5, 16, 32], [5, 5, 32, 32],
              [5, 5, 32, 32]]  # conv size x, conv size y, in channels, out channels
...
def test_dataset(dataset_x, dataset_y):
    ...
    for X_batch, Y_batch in batches(batch_size, dataset_x, dataset_y):
        loss_val_test, acc_val_test = sess.run([loss_op, accuracy_op],
                                               feed_dict={cnn_input: X_batch, y_correct: Y_batch})
